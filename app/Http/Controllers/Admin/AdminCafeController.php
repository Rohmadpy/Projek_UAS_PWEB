...
class AdminCafeController extends Controller
{
    ...
    {
        try {
            // Validasi dengan pesan error lebih jelas
            ...
            // Validasi primary_photo tidak melebihi jumlah foto
            $photoCount = count($request->file('photos'));
            if ($validated['primary_photo'] >= $photoCount) {
                return back()->withErrors(['primary_photo' => 'Index foto utama tidak valid. Maksimal: ' . ($photoCount - 1)])
                            ->withInput();
            }
            ...
            logger()->error('Error creating cafe: ' . $e->getMessage());
    ...
    public function update(Request $request, Cafe $cafe)
    {
        try {
            ...
            logger()->error('Error updating cafe: ' . $e->getMessage());
            return back()->withErrors(['error' => 'Terjadi kesalahan: ' . $e->getMessage()])
                        ->withInput();
        }
    }
    ...
    public function destroy(Cafe $cafe)
    {
        try {
            ...
            logger()->error('Error deleting cafe: ' . $e->getMessage());
            return back()->withErrors(['error' => 'Terjadi kesalahan: ' . $e->getMessage()]);
        }
    }
}